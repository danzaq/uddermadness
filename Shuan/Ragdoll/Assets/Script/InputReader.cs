...
public class InputReader : MonoBehaviour
{
    ...
    public static void GetXBoxControllerInput(string controllerIDString, ref bool up, ref bool down, ref bool left, ref bool right, ref bool fire, ref bool jump, ref bool dash, ref bool special)
    {
        ...
        if (IsBlocked)
            return;
        //
        fire = fire || (Input.GetButton("enter" + controllerIDString));
        jump = jump || Input.GetButtonDown("space" + controllerIDString);
        special = special || Input.GetButtonDown("E" + controllerIDString);
        //
        up = up || Input.GetAxisRaw("W" + controllerIDString) > 0.25f || Input.GetAxisRaw("L_YAxis" + controllerIDString) < -0.25f;
        down = down || Input.GetAxisRaw("S" + controllerIDString) < -0.25f || Input.GetAxisRaw("L_YAxis" + controllerIDString) > 0.25f;
        left = left || Input.GetAxisRaw("A" + controllerIDString) < -0.25f || Input.GetAxisRaw("L_XAxis" + controllerIDString) < -0.25f;
        right = right || Input.GetAxisRaw("D" + controllerIDString) > 0.25f || Input.GetAxisRaw("L_XAxis" + controllerIDString) > 0.25f;
        //
        dash = dash || Input.GetAxisRaw("F" + controllerIDString) > 0.25f || Input.GetAxisRaw("TriggersL" + controllerIDString) > 0.25f;
        //
    }
    //
